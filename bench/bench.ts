...
const encoders = {
  ...
  //
  "json-deduper": {
    encode: data => JSON.stringify(compress(data)),
    decode: data => decompress(JSON.parse(data))
  },
  "json-deduper w/ strings + numbers + MSGPACK": {
    encode: data => msgpack.encode(compress(data, {strings: true, numbers: true})),
    decode: data => decompress(msgpack.decode(data))
  },

  "json-deduper w/ strings + MSGPACK": {
    encode: data => msgpack.encode(compress(data, {strings: true})),
    decode: data => decompress(msgpack.decode(data))
  },

  "json-deduper w/ numbers + MSGPACK": {
    encode: data => msgpack.encode(compress(data, {numbers: true})),
    decode: data => decompress(msgpack.decode(data))
  },
};
...
for (let blob in blobs) {
  const result = results[blob] = {}
  const payload = require(blobs[blob])
  for (let key in encoders) {
    const solution = encoders[key];
